...
func TestPrepareCatalogWithMetaqueries(t *testing.T) {
	...
	catalog, _, requestCount, err := e.prepareCatalogWithMetaqueries(ctx, params, plan)
	require.NoError(t, err)
...
type metaqueryRunnerStub struct {
	sections []*metastore.DataobjSectionDescriptor
	requests []physical.MetastoreRequest
}

func (m *metaqueryRunnerStub) Run(_ context.Context, req physical.MetastoreRequest) (physical.MetastoreResponse, error) {
	m.requests = append(m.requests, req)
	return physical.MetastoreResponse{Kind: physical.MetastoreRequestKindSections, Sections: m.sections}, nil
}
